  ...
  */
    ...
    */
      async function retrieveAllListings() {
          ...
          try {
            const listings = await Listing.findAll(); // Uses finder method 'findAll' to retrieve all entries from the table
            console.log(JSON.stringify(listings)); // Outputs JSON of all entries to console
          }
          catch (error) {
            console.log(error); // Outputs error details to console
          }
      }
    /* 
   ...
   */
    async function findLibraryWest() {
      ...
      try {
        const libraryWest = await Listing.findOne({ // Uses finder method 'findOne' to retrieve entry with code 'LBW'
          where: { code: 'LBW' } 
        });
        console.log(libraryWest); // Outputs data for Library West to console
    ...
    */
      async function removeCable() {
        ...
        try {
          await Listing.destroy({ // Uses DELETE query to delete any entry with code 'CABL'
            where: {
              code: "CABL"
            }
          });
    ...
    */
    async function addDSIT() {
      ...
      try {
        const dsit = await Listing.create({ code: "DSIT", name: "Data Sciences and IT Building"}); // Uses INSERT query to create a new entry with given code & name
      }
      catch (error) {
        console.log(error); // Outputs error details to console
      }
    ...
    */
    async function updatePhelpsLab() {
      ...
      try {
        await Listing.update({ address: "1953 Museum Rd, Gainesville, FL 32603, United States" }, { // Uses UPDATE query to change address of entry w/ code 'PHL'
          where: {
            code: "PHL"
          }
        });
          const phelpsLab = await Listing.findOne({ // Uses finder method 'findOne' to retrieve entry with code 'LBW'
            where: { code: 'PHL' } 
          
        });
        console.log(phelpsLab); // Outputs Phelps Lab data to console
      }
      catch (error) {
        console.log(error); // Outputs error details to console
      }
    ...
    async function runFunctionsSequentially() {
      // Wrapped these in an async function since I was getting errors for too many DB connections. (Thanks Zachary Tyson!)
