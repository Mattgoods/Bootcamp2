 ...
 try {
  ...
  fs.readFile('listings.json', 'utf8', async function(err, data) {
    ...
    // to https://sequelize.org/docs/v6/core-concepts/model-instances/#creating-an-instance
     //ADD CODE HERE
     for (const entry of listingData.entries) { // Loops through each entry in listingData
      try {
        const newListing = await Listing.create({ // Uses INSERT query to create a new listing
          code: entry.code,
          name: entry.name,
        ...
        console.log(`Added listing with code: ${newListing.code}`);
      } catch (error) {
